...
class ActiveLearning:
    ...
    def add_samples_to_annotation_files(self, words):
        ...
        for file in text_files:
            containing_words = file.contains(excerpts=words)
            annotations = []
            annotation_file_name = file.file_name[:file.file_name.find(".")] + ".ann"
            for word_info in containing_words:
                annotations.append(Annotation(file_name=annotation_file_name, type=ANNOTATION_TYPE, begin=word_info[1], end=word_info[2], excerpt=word_info[0]))
            annotation_file = AnnotationFile(file_name=annotation_file_name, path=collection_path)
